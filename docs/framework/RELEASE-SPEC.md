...
- `diff`、`change`、`status` 与 `release` 的关系是什么
- 人类可读的发布日志应该如何从结构化记录派生

## 2. 基本判断

OpenDaaS 不应直接把 `diff` 自动拼接成 changelog，也不应要求开发端 Agent 每次自由重写整篇发布日志。
...
> **OpenDaaS 的 changelog 应该是结构化 release entry 的派生产物，而不是 diff 的自动拼接物。**
