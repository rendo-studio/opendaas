...
3. 如需 authored 版本文档，使用 `docPath` 显式指向文档包中的对应页面
...
- 持久化项目级版本记录
- 持久化版本记录对应 authored 文档的显式 `docPath`
- 承载结构化 highlights / breaking changes / migration notes / validation summary
- 作为未来对外发布说明或内部里程碑说明的稳定输入

OpenDaaS 不应假设这些文档固定落在 `docs/internal/versions/`，也不应自动把版本投影到特定目录。
...
> **开发端 Agent 或维护者应在“项目进入新的整体成熟状态并决定正式记录”时维护 `.opendaas/versions/records.yaml`，并在需要人类可读版本页时通过 `docPath` 显式绑定对应 authored 文档。**
